...
try:
    ...
    coll_topicos_nomes = db["topicos_nomes"]
    
    # ✅ LIMPA LISTA DE CONCLUÍDOS → RECOMEÇA DO ZERO!
    coll_topicos_concluidos.delete_many({})
    print("✅ Conectado ao MongoDB! ✅ Lista de concluídos LIMPA!")
...
def ja_foi_enviado(msg_id, nome, tam_bytes):
    return coll_enviados.find_one({
        "$or": [{"msg_id": str(msg_id)}, {"chave_unica": f"{nome}|{tam_bytes}"}]
    }) is not None
...
def salvar_enviado(msg_id, nome, tam_bytes, id_topico):
    coll_enviados.insert_one({
        "msg_id": str(msg_id), "nome": nome, "tamanho_bytes": tam_bytes,
        "chave_unica": f"{nome}|{tam_bytes}", "topico_id": id_topico, "enviado_em": time.time()
    })

# ==============================================
# ✅ MARCA TÓPICO CONCLUÍDO
# ==============================================
...
def marcar_topico_concluido(nome_topico, id_topico, total_itens):
    coll_topicos_concluidos.update_one(
        ...
        {"$set": {"topico_id": id_topico, "total_itens": total_itens, "concluido_em": time.time()}},
        upsert=True
    )

# ==============================================
# ✅ DESCOBRE ID REAL DO TÓPICO
# ==============================================
async def descobrir_id_topico(app, nome_topico):
    salvo = coll_topicos_nomes.find_one({"nome": nome_topico})
    if salvo: return salvo["topico_id"]

    nome_busca = re.sub(r"álbum\s+", "", nome_topico, flags=re.IGNORECASE).strip().lower()
    contagem_ids = {}

    async for msg in app.get_chat_history(GRUPO_ORIGEM, limit=3000):
    ...
    if contagem_ids:
        ...
        print(f"🔍 ID DESCOBERTO: '{nome_topico}' → {id_real}")
        return id_real
    print(f"⚠️ ID NÃO ENCONTRADO: {nome_topico}")
    return None
...
async def processar_topico(app, nome_topico):
    ...
    while True:
        ...
        async for msg in app.get_chat_history(GRUPO_ORIGEM, limit=lote_tam, offset_id=ultimo_id):
            ultimo_id = msg.id
            tid = getattr(msg, 'topic_id', None) or getattr(msg, 'reply_to_message_id', None)
            if tid != id_topico: continue
            if not msg.video and not msg.photo: continue
            lote.append(msg)
        if not lote: break
        todas_msg.extend(lote)
        if len(lote) < lote_tam: break
    ...
    for msg in todas_msg:
        ...
        if not eh_video and not eh_foto: continue
        ...
        item = {"msg_id": msg_id, "nome": nome, "tam_bytes": tam_bytes,
                "midia": midia, "tipo": "VÍDEO" if eh_video else "FOTO",
                "legenda": legenda_final, "grupo_id": tem_grupo}

        if ja_foi_enviado(msg_id, nome, tam_bytes):
            if grupo_atual: grupos.append(grupo_atual); grupo_atual = []
            continue
        ...
        if tem_grupo:
            if not grupo_atual or grupo_atual[0]["grupo_id"] == tem_grupo:
                ...
                grupos.append(grupo_atual); grupo_atual = [item]
        else:
            if grupo_atual: grupos.append(grupo_atual); grupo_atual = []
            grupos.append([item])
    ...
    for grupo in grupos:
        ...
        if qtd > 1: print(f"📦 [{contador}/{len(grupos)}] AGRUPAMENTO DE {qtd} MÍDIAS")
        else: print(f"📤 [{contador}/{len(grupos)}] {grupo[0]['tipo']} {tam_mb} → {grupo[0]['nome'][:35]}")

        try:
            caminhos = []
            for item in grupo:
                cam = await app.download_media(item["midia"], PASTA_TEMP + item["nome"])
                if not cam or os.path.getsize(cam)==0: continue
                caminhos.append({**item, "caminho": cam})
            if not caminhos: print(f"⚠️ Arquivo vazio → pulando\n"); continue

            if len(caminhos) > 1:
                lote_midia = []
                for i,m in enumerate(caminhos):
                    leg = legenda_grupo if i==0 else ""
                    if m["tipo"]=="FOTO": lote_midia.append(InputMediaPhoto(m["caminho"], caption=leg))
                    else: lote_midia.append(InputMediaVideo(m["caminho"], caption=leg, supports_streaming=True))
                await app.send_media_group(GRUPO_DESTINO, media=lote_midia)
                print(f"✅ ENVIADO AGRUPADO!\n")
            else:
                m = caminhos[0]
                if m["tipo"]=="VÍDEO": await app.send_video(GRUPO_DESTINO, m["caminho"], caption=m["legenda"], supports_streaming=True)
                else: await app.send_photo(GRUPO_DESTINO, m["caminho"], caption=m["legenda"])
                print(f"✅ ENVIADO!\n")

            for item in grupo: salvar_enviado(item["msg_id"], item["nome"], item["tam_bytes"], id_topico)
    ...
    print(f"\n🏁 CONCLUÍDO! Enviados: {total_enviados} itens\n")
...
async def main():
    ...
    async with Client("minha_conta", API_ID, API_HASH, phone_number=TELEFONE) as app:
        ...
        total_geral = 0
        for nome in TOPICOS_PARA_PROCESSAR:
            qtd = await processar_topico(app, nome)
            total_geral += qtd
            print(f"⏳ Pausa 3s...\n")
